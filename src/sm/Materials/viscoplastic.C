...
// constructor
ViscoPlastic :: ViscoPlastic(int n, Domain *d) : StructuralMaterial(n, d), elementSet(0, d)
{
...
ViscoPlastic :: initializeFrom(InputRecord *ir)
{
    ...
    lobs = 0.0;
    IR_GIVE_OPTIONAL_FIELD(ir, lobs, _IFT_ViscoPlastic_lobs); /// Internal Length scale for ViscoPlastic law
    
    
    //Domain *domain = this->giveDomain();
    NodalRecoveryModel :: NodalRecoveryModelType stype;
    stype = (NodalRecoveryModel :: NodalRecoveryModelType) 1;
    if (this->smoother == NULL) {
      this->smoother = classFactory.createNodalRecoveryModel(stype, this->giveDomain());
    }
    
    this->elementSet.addAllElements();
                                        ...
                                        TimeStep *tStep)
{
    ...
    double xita;
    this->computeXitaFromNodes(xita, gp, tStep);
...
void
ViscoPlastic::computeXitaFromNodes(double& xita, GaussPoint* gp, TimeStep* tStep)
{
    ...
    //// Calculate Gradient of Stresses at gp 
    FEInterpolation *interp = elem->giveInterpolation();
    FloatMatrix dN;
    interp->evaldNdx( dN, gp->giveNaturalCoordinates(), FEIElementGeometryWrapper(elem) );
    int ncomp = dN.giveNumberOfColumns();
    
    /// Compute equivalent stress at nodes
    int nnodes = elem->giveNumberOfNodes();
    double seq;
    const FloatArray *nodeStress; 
    FloatArray stresseq(nnodes);
    for ( int i = 1; i <= nnodes; i++){
      int nodenum = elem->giveNode(i)->giveNumber();
      this->smoother->giveNodalVector(nodeStress, nodenum);
      FloatArray stress(nodeStress->giveSize());
      stress = *nodeStress;
      seq =  computeStressNorm(stress);
      
      stresseq.at(i) = seq;
    }
    FloatArray XitaVec(ncomp);
    XitaVec.beTProductOf(dN,stresseq);
...
//     double tempxita;
//     tempxita = 0.0;
//     if (status->giveEquivalentStress() > 1.e-6) {
//       tempxita = xita/status->giveEquivalentStress();	
//     }
//     if (tempxita > 4.0) {
//       xita = 4.0;
//     } else {
...
ViscoPlastic :: giveIPValue(FloatArray &answer, GaussPoint *gp, InternalStateType type, TimeStep *tStep)
{
    ...
    if ( type == IST_PlasticStrainTensor ) {
        ...
        answer.resize(1);
        answer.at(1) = status->giveEquivalentStress();
        return 1;     
...
void ViscoPlastic::updateYourself(TimeStep* tstep)
{
  
    this->smoother->recoverValues(this->elementSet, IST_StressTensor, tstep);

  //     for (auto &s : domain->giveSets()){
//       this->smoother->recoverValues(s->get(), IST_StressTensor, tstep);
//     }
    return;
}
...
double
ViscoPlastic :: computeStrainNorm(const FloatArray &s)
{
    if ( s.giveSize() == 1 ) {
        return fabs(s [ 0 ]);
    }
