...
ViscoPlasticNl :: computeXita(double &xita, GaussPoint *gp, TimeStep *tStep)
{
    ...
    status->setTempStressGradient(nonlocalXita);
   
    double stresseq = status->giveEquivalentStress();
    if (stresseq > 1.e-15) {
      if (stresseq > 0.01*sig0) {
...
//    nonlocalXita *= 1./(status->giveEquivalentStress() + 1.e-12);
    if (this->cl > 0.0) {
      nonlocalXita *= cl;
    } else {
      nonlocalXita = 0.;
...
int
ViscoPlasticNl :: giveIPValue(FloatArray &answer, GaussPoint *gp, InternalStateType type, TimeStep *tStep)
{
    ViscoPlasticNlStatus *status = static_cast< ViscoPlasticNlStatus * >( this->giveStatus(gp) );
    if ( type == IST_StGradient ) {
	answer.resize(1);
        answer.at(1) = status->giveStressGradient();
        return 1;
    } else {
        return ViscoPlastic :: giveIPValue(answer, gp, type, tStep);
    }
}
...
//
{
    StressGradient = tempStressGradient;
