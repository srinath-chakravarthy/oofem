...
comm_Memory::comm_Memory(MPI_Comm comm)
{
  error = new comm_Error(comm);
}
...
void *comm_Memory::smalloc(int n, const char *name)
{
  if (n == 0) return NULL;
  void *ptr = malloc(n);
  if (ptr == NULL) {
...
void comm_Memory::sfree(void *ptr)
{
  if (ptr == NULL) return;
  free(ptr);
}
...
void *comm_Memory::srealloc(void *ptr, int n, const char *name)
{
  if (n == 0) {
    sfree(ptr);
    return NULL;
...
double **comm_Memory::create_2d_double_array(int n1, int n2, const char *name)
...
double **comm_Memory::grow_2d_double_array(double **array,
...
void comm_Memory::destroy_2d_double_array(double **array)
